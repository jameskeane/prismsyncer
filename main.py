...
class Syncer:
    ...
    def handle_file_synced(self, folder, filename):
        if not folder in self.mappings: return
        ...
        self.webdav.upload_sync(remote_path=f"import/{filename}", local_path=fullpath)
        return True
    ...
    def run(self):
        ...
        event_stream = self.syncthing.events(filters=("ItemFinished",), last_seen_id=last_id)
